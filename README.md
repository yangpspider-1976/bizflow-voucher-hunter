...
- **Levels, missions and achievements**: five XP levels bought with LP, time-windowed daily missions, partner-run urgent campaigns with quota, budget and evidence review, tiered permanent badges, a central idempotent reward engine, and versioned economy settings an operator changes without a deploy (see `docs/GAMIFICATION.md`)
- **Gamification operations**: a Partner CMS with a pre-flight cost simulation and an approval workflow, nightly anomaly detection with graduated reward holds, a KPI dashboard, and a partner statement that separates mission rewards and level conversions from what is billed
